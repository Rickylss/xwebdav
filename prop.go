...
var liveProps = map[xml.Name]struct {
	...
	{Space: "DAV:", Local: "quota-available-bytes"}: {
		findFn: findAvailableBytes,
		dir: true,
	},
	{Space: "DAV:", Local: "quota-used-bytes"}: {
		findFn: findUsedBytes,
		dir: true,
	},
}
...
func findAvailableBytes(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi os.FileInfo) (string, error) {
	return fmt.Sprintf(`%d`, fi.Size()), nil
}

func findUsedBytes(ctx context.Context, fs FileSystem, ls LockSystem, name string, fi os.FileInfo) (string, error) {
	return fmt.Sprintf(`%d`, fi.Size()), nil
}
